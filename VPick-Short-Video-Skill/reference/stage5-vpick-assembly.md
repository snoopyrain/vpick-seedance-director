...
### Step 5.4 — 填入每個 video node 的 prompt

prompt 結構(融合所有 shots + 對白 + 環境平面圖描述):

```
{visual_style} style, {environments[0].time_of_day}, {environments[0].location}.
Spatial layout: {environments[0].floor_plan_note}

Sequence ({total_duration_seconds} seconds total, N shots):
{for each shot:}
  ({shot.time}, {shot.duration_seconds}s) {shot.shot_size}: {shot.visual_content}
    {shot.character} {shot.action}
    Lighting: {shot.lighting}
    Audio: {shot.audio}
    {if shot.dialogue:}
      Dialogue ({shot.dialogue_speaker}): "{shot.dialogue}"

Color tone: {environments[0].color_tone}

DIALOGUE VOICE:
{if speaking_characters:}
  Use the attached voice reference audio file(s) as the speaker's voice tone and accent.
  Speaker {character_b}: use voice reference #1
  Speaker {character_a}: use voice reference #2

CONSISTENCY: Maintain exact character appearance, costume, environment spatial layout, and lighting as shown in reference images. Do not redesign visual elements.
```
