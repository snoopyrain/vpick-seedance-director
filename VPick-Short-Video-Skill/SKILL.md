---
name: vpick-seedance-director
description: VPick Seedance 導演級短劇製作助手 — 把故事腳本透過 5 階段流程(設定 → 拆腳本 → 角色/環境 reference → 分鏡表 → Seedance 2 影片)轉成完整短劇,所有節點都建在 VPick 專案內,並用電影導演工法寫每段 Seedance prompt(整合 Higgsfield Seedance Shotlist Director)。觸發條件:當使用者說「做分鏡」、「做短劇」、「storyboard」、「把這個故事做成影片」、「用 VPick 做影片」、「Seedance 短劇」、「Seedance 導演」、「分鏡表」、「角色 reference 圖」、「環境 reference」、貼上故事腳本要求做成影片時使用此 skill。
---

# VPick Seedance Director Skill — 5 階段短劇前製到導演級影片產出
...
| `reference/seedance-director-craft.md` | **影片導演大腦**:Style Prefix + CUT 結構 + 導演工法 + 範例 | 寫 Stage 2 shots / Stage 5 video prompt 前必讀 |
...
7. **導演工法貫穿 Stage 2 / 4 / 5**:拆鏡頭依戲劇節奏、分鏡表寫 blocking 與表演 beat、video prompt 用 `STYLE PREFIX → Characters → Scene → CUT` 結構。影片風格鎖定 **8K 電影寫實**。全部細節見 `reference/seedance-director-craft.md`(整合自 Higgsfield Seedance Shotlist Director 的導演工法)
...
4. **寫 Stage 2 shots 或 Stage 5 video prompt 前,先讀 `reference/seedance-director-craft.md`**(導演工法是這個 skill 影片品質的核心)
5. Stage 3/4/5 並行生成時,`mcp__vpick__run_image_generator` / `run_video_generator` 同時起跑,完成一張立即呈現
